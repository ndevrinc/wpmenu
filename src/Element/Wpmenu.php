...
namespace Drupal\wpmenu\Element;
 ...
 */
class Wpmenu extends RenderElement {
   ...
   */
  public static function preRenderWpmenu($element) {
    // Get the configured breakpoints to switch from vertical to horizontal
    // toolbar presentation.
    $breakpoints = static::breakpointManager()->getBreakpointsByGroup('wpmenu');
    if (!empty($breakpoints)) {
      ...
      $element['#attached']['drupalSettings']['wpmenu']['breakpoints'] = $media_queries;
    }

    $module_handler = static::moduleHandler();
    // Get toolbar items from all modules that implement hook_toolbar().
    $items = $module_handler->invokeAll('wpmenu');
    // Allow for altering of hook_toolbar().
    $module_handler->alter('wpmenu', $items);
    // Sort the children.
    ...
    foreach (Element::children($element) as $key) {
      $element[$key]['#id'] = Html::getId('wpmenu-item-' . $key);
    }
